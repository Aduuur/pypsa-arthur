...
=============================================
Analysezeitraum aus config.DARK_SKY_START / DARK_SKY_END.
Unterstützt perfect-foresight (MultiIndex) UND myopic (DatetimeIndex).

Funktioniert im ARO-Modus und im normalen Modus.
Ausgabe in config.PLOT_OUTPUT_PATH/heat_diagnosis/.

ARO-Fix: Jahr wird aus n.snapshots gelesen statt aus Dateinamen-Regex,
der bei Dispatch-Netzen kein Match liefert.
"""
...
def _cop_check(n, links_idx, label, p0, p1, mask):
    ...
    print(f"     Beispiel-Link: '{ex}'  bus0={bus0}  bus1={bus1}  efficiency={eff:.3f}")
    ...
    if in_p1:
        ...
        if sum_p0 > 0 and sum_p1 > 0:
            # COP-Richtung hängt von bus0/bus1 ab:
            # WP in PyPSA-Eur: bus0=Wärme(Output), bus1=Strom(Input)
            #   → COP = Wärme/Strom = p0/p1
            # Heizstab:        bus0=Strom(Input),  bus1=Wärme(Output)
            #   → Wirkungsgrad ≈ 1 = p1/p0
            bus0_val = n.links.at[links_idx[0], "bus0"] if links_idx[0] in n.links.index else ""
            if "heat" in str(bus0_val).lower():
                cop = sum_p0 / sum_p1
                ok  = 1.5 <= cop <= 6.0
                print(f"     => COP (Wärme/Strom = p0/p1): {cop:.2f}  "
                      f"{'✅  plausibel (1.5–6.0)' if ok else '⚠️  außerhalb 1.5–6.0'}")
            else:
                cop = sum_p1 / sum_p0
                ok  = 0.8 <= cop <= 1.1
                print(f"     => Wirkungsgrad (p1/p0): {cop:.2f}  "
                      f"{'✅  plausibel (0.8–1.1)' if ok else '⚠️  außerhalb 0.8–1.1'}")
    else:
        print("     ❌ Kein p1 verfügbar")

    ts_p0   = p0[in_p0].loc[mask].abs().sum(axis=1)
    mean_p0 = ts_p0.mean()
    cv = ts_p0.std() / mean_p0 if mean_p0 > 0 else 0
    if cv < 0.02:
        cv_note = "konstanter Betrieb (bei DF-Analyse normal)"
    else:
        cv_note = "✅  zeigt Variabilität"
    print(f"     CV = {cv:.3f}  (mean={mean_p0/1000:.3f} GW)  {cv_note}")
...
def analyze_heat_sector(n, country, ts_index, mask):
    ...
    plot_idx = pd.DatetimeIndex(
        ts_index.get_level_values(-1)[mask]
        if isinstance(ts_index, pd.MultiIndex)
        else ts_index[mask]
    )
...
def plot_diagnosis(p_elec, q_heat, country, target_year, scenario_name,
    ...
    ax2.set_ylabel("Wärmeerzeugung [GW$_{th}$]")
    ax2.set_title("Wärmebereitstellung nach Technologie")
...
def main():
    ...
    # FIX: PLOT_OUTPUT_PATH statt BASE_SAVE_PATH
    save_dir = os.path.join(config.PLOT_OUTPUT_PATH, "heat_diagnosis")
    ...
    for scenario_name, paths in networks.items():
        ...
        for path in paths:
            ...
            try:
                ...
                # FIX: Jahr immer aus Snapshots lesen — Dispatch-Netze haben
                # kein ___YYYY im Namen, Regex würde None zurückgeben
                target_year = str(_parse_year(n, path, fallback=2050))
            ...
            print(f"  Snapshots: {'MultiIndex' if is_multi else 'DatetimeIndex'}")
            ...
            if n_steps == 0:
                print("  ⚠️  Kein Zeitfenster → überspringe")
                continue
