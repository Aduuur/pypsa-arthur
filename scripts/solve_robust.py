...
def stack_scenarios_to_multisnapshot_network(
    ...
    for t_container_name, attrs_meta in scan_meta.items():
        ...
        for attr, col_meta in attrs_meta.items():
            ...
            # [BUG-FIX-12] Sort constant_vals together with constant_canon_cols
            # to avoid value/column mismatch after sorting.
            if constant_canon_cols:
                paired = sorted(
                    zip(constant_canon_cols, constant_vals),
                    key=lambda xv: (
                        static_norm_order.index(xv[0]) if xv[0] in static_norm_order else 10**9,
                        xv[0],
                    ),
                )
                constant_canon_cols, constant_vals = map(list, zip(*paired))
            else:
                constant_canon_cols = []
                constant_vals = []
...
def _dispatch_solve(
    ...
    def extra_dispatch(network: pypsa.Network, snapshots: pd.Index) -> None:
        ...
        m.objective = op_cost
        # [DISPATCH-FIX-TES] Enforce TES energy-to-power-ratio constraint in dispatch.
        # Without this, e_nom_extendable stores are unconstrained and the optimizer
        ...
        # [DISPATCH-FIX-2] Drop investment-type GCs in dispatch
        # lv_limit and similar constraints are investment constraints
        # and should not be active when no investment decisions are made
        _inv_gc_types = {"transmission_volume_expansion_limit",
                         "transmission_expansion_limit"}
        _to_drop = [gc for gc, row in network.global_constraints.iterrows()
                    if str(row.get("type","")) in _inv_gc_types]
        if _to_drop:
            network.global_constraints.drop(_to_drop, inplace=True)
            logger.info("[DISPATCH-FIX-2] Dropped %d investment GCs: %s",
...
def _fix_unbounded_infrastructure(n: pypsa.Network) -> Dict[str, int]:
    ...
    if hasattr(n, "links") and len(n.links) > 0:
        ...
        inf_max = lk.get("p_nom_max", np.inf).fillna(np.inf) >= 1e5  # 1e6 ist pypsa-Dummy fuer "unbegrenzt"
        mc_pos = lk.get("marginal_cost", 0).fillna(0).astype(float) > 0
        # NEU:
        low_cc = lk.get("capital_cost", 0).fillna(0).astype(float) < 10.0

        to_fix = lk.index[is_ext & inf_max & mc_pos & low_cc]
        if len(to_fix) > 0:
            # Korrekte capital_cost je Carrier berechnen statt pauschale 1 EUR/MW
            # water pit charger/discharger: Kosten liegen im Store (EUR/kWh)
            # = energy_to_power_ratio × store_capital_cost = 22.5h × 81.68 EUR/kWh = 1838 EUR/MW
            WP_CC = 1837.8  # EUR/MW
            # [BUG-FIX-13] Only charger gets WP_CC, discharger stays at 1 EUR/MW
            # (consistent with battery: only charger carries investment cost)
            wp_charger_mask = lk.loc[to_fix, "carrier"].str.contains("water pits charger", case=False).values
            wp_discharger_mask = lk.loc[to_fix, "carrier"].str.contains("water pits discharger", case=False).values
            n.links.loc[to_fix[wp_charger_mask], "capital_cost"] = WP_CC
            n.links.loc[to_fix[wp_discharger_mask], "capital_cost"] = 1.0
            n.links.loc[to_fix[~wp_charger_mask & ~wp_discharger_mask], "capital_cost"] = 1.0
            # [WP-CAP] Cap water pit charger/discharger p_nom_max to 1 GW per bus
            # (realistic district heating potential; prevents solver from over-investing)
            wp_all_mask = wp_charger_mask | wp_discharger_mask
            n.links.loc[to_fix[wp_all_mask], "p_nom_max"] = 100.0
            logger.info("[WP-CAP] Capped %d water pit charger/discharger to p_nom_max=0.1 GW", int(wp_all_mask.sum()))
            fixed["links::zero_cc"] = len(to_fix)
            logger.info(
                "[INFRA-FIX] Fixed capital_cost for %d links "
                "(water pit → %.0f EUR/MW, others → 1 EUR/MW). Carriers: %s",
                len(to_fix), WP_CC,
...
def solve_aro_master(
    ...
    # [NUM-FIX-0b5] Cap water tanks/pits e_nom_max via energy-to-power-ratio.
    # Without this, the optimizer builds ~1 TWh thermal storage per node which
    # causes unrealistic dispatch (charge/discharge 2800+ times/year).
    # Correct cap: e_nom_max = p_nom_max_charger * energy_to_power_ratio.
    # Water pit charger is capped at 100 MW [WP-CAP], tanks use same logic.
    _wt_mask = (
        n.stores.carrier.str.contains("water tank|water pit", case=False) &
        n.stores.e_nom_extendable.fillna(False)
    )
    _wt_stores = n.stores[_wt_mask].index
    if len(_wt_stores) > 0:
        for store_name in _wt_stores:
            store = n.stores.loc[store_name]
            bus = store["bus"]
            # find charger link for this store bus
            charger_mask = (
                n.links.bus1 == bus
            ) & n.links.carrier.str.contains("charger", case=False)
            if charger_mask.any():
                charger = n.links[charger_mask].iloc[0]
                etp = charger.get("energy to power ratio", None)
                p_max = charger.get("p_nom_max", np.inf)
                if etp is not None and not np.isnan(etp) and etp > 0:
                    if np.isinf(p_max):
                        # fallback: 500 MW per node is a generous upper bound
                        p_max = 500.0
                    e_max = float(etp) * float(p_max)
                    n.stores.loc[store_name, "e_nom_max"] = e_max
                    continue
            # fallback if no charger found: cap at 1 TWh
            n.stores.loc[store_name, "e_nom_max"] = 1e6
        logger.info(
            "[NUM-FIX-0b5] water tank/pit e_nom_max set via energy-to-power-ratio (%d stores). "
            "Sample e_nom_max: %s",
            len(_wt_stores),
            {s: f"{n.stores.loc[s,'e_nom_max']:.0f} MWh"
             for s in list(_wt_stores)[:4]},
        )
...
def run_robust(
    ...
    # [FIX] Kapazitaeten aus gespeichertem Netzwerk lesen
    # Das gestapelte Netzwerk hat p_nom_opt als Summe ueber Szenarien.
    try:
        import pypsa as _pypsa
        _n_saved = _pypsa.Network(out_network)
        capacities = extract_capacities(_n_saved)
        del _n_saved
        logger.info("[FIX] capacities aus gespeichertem Netzwerk: %s", out_network)
    except Exception as _ce:
        logger.warning("[FIX] capacities fallback auf gestapeltes Netzwerk: %s", _ce)
